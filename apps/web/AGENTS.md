...
- **Auth gating**: `src/features/auth/require-auth.tsx` and `src/app/auth-boundary.tsx` gate signed-in-only routes; sign-in itself redirects to the separate `apps/auth` hub via `VITE_AUTH_APP_URL`, not an in-app flow.
