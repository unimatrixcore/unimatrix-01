...
- **Auth boundary**: Route handlers must read the acting user only via `getAuthUserId` (from `@unimatrix/auth/server`, backed by a verified Clerk session) — never from client-supplied ids or body fields.
