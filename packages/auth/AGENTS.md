...
- `src/server.ts`: the `./server` entry point — `registerClerkAuth` (wraps `@clerk/fastify`'s `clerkPlugin`), `requireAuth`/`requirePermission` preHandler guards, `getAuthUserId`, `getSessionPermissionsClaim`, and the `AuthError` class. Node-only.
...
- **No dependency on `@unimatrix/shared`**: the dependency direction stays one-way (`apps/api` depends on both). Keep this package free of `@unimatrix/shared` imports so it never risks a cycle.
